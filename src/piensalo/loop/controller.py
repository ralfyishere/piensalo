...
from piensalo.loop.state import (
    LoopPaths,
    jload,
    jsave,
    now,
...
def generate_next_prompt(paths: LoopPaths, st: dict) -> str:
    ...
    lines: list[str] = []
    add = lines.append
    add(f"# NEXT PROMPT — generated {now()} by piensalo loop from canonical state")
    add("")
    add("## Goal contract")
    add(f"- {st.get('goal')}")
    add("")
    ...
    add(
        ...
        "model in .piensalo/session-provenance.json BEFORE work."
    )
    add(
    ...
    add("## Bounds")
    add(
        f"- ONE atomic cycle per `piensalo loop step`; <={MAX_CYCLES_PER_SESSION} "
...
def cmd_session_start(paths: LoopPaths, args: list[str]) -> None:
    ...
    if not st:
        _die("no loop state — run: piensalo loop start GOAL.md")
...
def cmd_session_end(paths: LoopPaths, args: list[str]) -> None:
    ...
    print(
        ...
        "resume: piensalo loop session-start && piensalo loop resume"
    )
...
def cmd_status(paths: LoopPaths) -> None:
    ...
    if cap:
        print(
            ...
            f"{MAX_CYCLES_PER_SESSION}) — a NEW session runs: piensalo loop session-start"
        )
...
def cmd_step(paths: LoopPaths, args: list[str]) -> None:
    ...
    if st.get("open_cycle"):
        _die(
            ...
            "piensalo loop step close OUTCOME.json"
        )
    ...
    print(
        ...
        f"{cyc['budget_wall_min']} min · close with: piensalo loop step close OUTCOME.json ==="
    )
...
def cmd_step_close(paths: LoopPaths, st: dict, outcome_path: str | None) -> None:
    ...
    if not outcome_path or not Path(outcome_path).exists():
        _die("outcome file required: piensalo loop step close OUTCOME.json")
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for ``piensalo loop ...``; returns an exit code."""
    ...
    try:
        if cmd == "start":
            if not rest:
                _die("usage: piensalo loop start GOAL.md")
        ...
        print(f"piensalo loop: {e}", file=sys.stderr)
        return 1
    return 0
