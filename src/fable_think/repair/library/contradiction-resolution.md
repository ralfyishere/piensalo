...
description: "When inputs contain conflicting facts, stop and resolve the conflict explicitly — classify it, pick a resolution rule, and log what stays unresolved — before building anything on either side."
---
...
## Verification
Every detected conflict appears in the log; no conclusion silently builds on one side of an unresolved contradiction.
...
**Evidence status:** experimentally supported (n=1, controlled transfer eval) — procedure grounded in documented live-state-over-plan-markers and unresolved-contradiction failure modes.
