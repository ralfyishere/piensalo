...
In scope: the `fable-think` CLI and runtime, the skill loader and
...
| **No hidden persistence** | All state lives in `.fable-think/` in the workspace (or an explicitly configured path). Nothing is written elsewhere. |
| **No auto-publish** | Nothing is pushed, published, or shared without an explicit command and confirmation. |

## Tooling

- **`fable-think skill scan <path-or-package>`** — vets a skill before
...
- **`fable-think doctor`** — audits the local installation: verifies the
