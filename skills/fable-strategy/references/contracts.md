...
**Failure it repairs:** Trades breadth for depth (rich schema per candidate, fewer candidates) or depth for breadth (long shallow lists); systematically misses the mechanisms furthest from the evidence log's surface wording (R2/R4 class: 'the fix pattern already exists in-repo, apply it here').
...
**Procedure:**
1. Pair every transferred contract with one full worked exemplar drawn from curated expert reasoning traces, not a paraphrase.
