...
|---|---|---|---|
| 1 | Fresh clone works | On a machine with no prior exposure: `git clone` → documented install → `fable-think doctor` passes, ≤ 10 minutes | recorded session log |
| 2 | One-command install | Each of `uvx fable-think`, `pipx install fable-think`, `npx skills add fable-think` succeeds on a clean environment | CI or recorded logs, one per path |
...
| 7 | Scans pass | `fable-think skill scan` clean on all shipped skills; secret scan clean on full git history; no private identifiers or machine paths in the repo | scan outputs archived |
